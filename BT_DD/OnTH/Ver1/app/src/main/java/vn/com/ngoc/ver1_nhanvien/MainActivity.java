package vn.com.ngoc.ver1_nhanvien;
...
public class MainActivity extends AppCompatActivity {
    ...
    Button thoat, them, xoa, sua, select, trang,tim;
    GridView dsNhanVien;
    DBHelper dbHelper;
        ...
        tim.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(id.getText().toString().equals("")){
                    Toast.makeText(getApplicationContext(), "Nhập id cần tìm", Toast.LENGTH_SHORT).show();
                }
                else {
                    tim(Integer.parseInt(id.getText().toString()));
                }
            }
        });
    ...
    public  void  tim(int id){
        NhanVien nhanVien= dbHelper.getNhanVientheoId(id);
        if(nhanVien==null){
            Toast.makeText(getApplicationContext(), "Không tìm thấy nhân viên có id-"+id, Toast.LENGTH_SHORT).show();
        }
        else {
            ArrayList<String> list= new ArrayList<>();
            list.add(nhanVien.getId()+"");
            list.add(nhanVien.getHoten());
            list.add(nhanVien.getSdt()+"");
            list.add(nhanVien.getDiachi());
            ArrayAdapter<String> adapter= new ArrayAdapter<>(this, android.R.layout.simple_list_item_1, list);

            dsNhanVien.setAdapter(adapter);

            Toast.makeText(getApplicationContext(), "Tìm thành công", Toast.LENGTH_SHORT).show();
        }
